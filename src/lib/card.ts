 ...
 *
 * SECURITY: values produced/consumed here are used ONLY in the browser for
 * display, validation, and client-side tokenization. The full card number,
 * expiry, and CVV are NEVER sent to our API and NEVER logged.
 */
...
export function billingZipError(zip: string): string | null {
  ...
  if (!trimmed) return "Billing postal / ZIP code is required";
  if (CA_POSTAL.test(trimmed) || US_ZIP.test(trimmed)) return null;
  return "Enter a valid Canadian postal code or US ZIP code";
}
